...
full_data <- suppressMessages(read_csv(here("data", "raw_data", "20220714_TimepointDB_PrimaryvChronic.csv"))) %>% filter(., Selected_Final == 'Yes') %>%
...
full_data <- full_data %>%
  mutate(Outcome_Group = case_when(is.na(Nr_Relapses_PreLeisH) ~ as.character(Group),
                                   Nr_Relapses_PreLeisH == 0 ~ "Cured_VL_HIV",
                                   Nr_Relapses_PreLeisH > 0 ~ "Relapse_VL_HIV")) %>% relocate(Outcome_Group, .after = Group)
...
boxplot_data <- boxplot_data %>% mutate(Aggregated_Group = case_when(Outcome_Group %in% c('HIV', 'AL_HIV') ~ Outcome_Group,
                                                                     Outcome_Group %in% c('Cured_VL_HIV', 'Relapse_VL_HIV') ~ 'VL_HIV'))

boxplot_data <- boxplot_data %>% mutate_if(is.character, factor)

boxplot_data$Outcome_Group <- factor(boxplot_data$Outcome_Group, levels = c('HIV', 'AL_HIV', 'Cured_VL_HIV', 'Relapse_VL_HIV'))
...
timeseries_data <- full_data %>% filter(., Outcome_Group %in% c('Cured_VL_HIV', 'Relapse_VL_HIV')) %>% filter(., !str_starts(RecodedTimepoint, 'Pre_')) %>% mutate_if(is.character, factor)
timeseries_data$Outcome_Group <- forcats::fct_drop(timeseries_data$Outcome_Group)

timeseries_data$Outcome_Group <- factor(timeseries_data$Outcome_Group, levels = c('Cured_VL_HIV', 'Relapse_VL_HIV'))
...
MWU_between_primary_chronic <- sapply(used_markers, function(x) {
  boxplot_data_only_primary_chronic <- filter(boxplot_data, Outcome_Group %in% c('Cured_VL_HIV', 'Relapse_VL_HIV'))
  boxplot_data_only_primary_chronic$Outcome_Group <- forcats::fct_drop(boxplot_data_only_primary_chronic$Outcome_Group)
  mwu_primary_chronic <- boxplot_data_only_primary_chronic %>% wilcox.test(unlist(.[x]) ~ Outcome_Group, data = .)
...
mixed_effects_models <- sapply(markers[-43], function(x) {
  lmer_data <- timeseries_data %>% .[x] %>% pull
  lmer_test <- lmer(lmer_data ~ Outcome_Group*RecodedTimepoint_in_Days + (1|Patient_SampleID), data = timeseries_data)
}, simplify = FALSE, USE.NAMES = TRUE)

residual_plots <- sapply(mixed_effects_models, function(x) {
  ggqqplot(residuals(x))
}, simplify = FALSE, USE.NAMES = TRUE)

anova_of_models <- sapply(mixed_effects_models, function(x) {
  anova(x)
}, simplify = FALSE, USE.NAMES = TRUE)

Group_pvals <- sapply(anova_of_models, function(x) {
  x$`Pr(>F)`[1]
}, simplify = T, USE.NAMES = T)

Timepoint_pvals <- sapply(anova_of_models, function(x) {
  x$`Pr(>F)`[2]
}, simplify = T, USE.NAMES = T)

Interaction_pvals <- sapply(anova_of_models, function(x) {
  x$`Pr(>F)`[3]
}, simplify = T, USE.NAMES = T)

significant_group <- Group_pvals[Group_pvals < 0.05]
significant_interaction <- Interaction_pvals[Interaction_pvals < 0.05]
...
Cross_sectional_boxplot_generator <- function(marker_list, step_size_list) {
  ...
  boxplot_list <- lapply(seq_along(marker_list), function(x) {
    ...
    temp_data <- temp_data %>% filter(., Outcome_Group %in% c('Cured_VL_HIV', 'Relapse_VL_HIV'))
    temp_data$Outcome_Group <- forcats::fct_drop(temp_data$Outcome_Group)
    temp_data$Aggregated_Group <- temp_data$Outcome_Group
    
    combined_data <- bind_rows(boxplot_data, temp_data)
    
    combined_data$Aggregated_Group <- factor(combined_data$Aggregated_Group,
                                             levels = c('HEC', 'HIV', 'AL_HIV', 'VL_HIV', 'Cured_VL_HIV', 'Relapse_VL_HIV'))
    ...
    overall_boxplot <- combined_data %>% filter(., !is.na(.data[[tmp]])) %>%
      ...
      geom_signif(textsize = 8, comparisons = list(c('Cured_VL_HIV', 'Relapse_VL_HIV')), annotations = format(MWU_between_primary_chronic[[tmp]], digits = 1, nsmall = 3)) +
...
Longitudinal_timeseries_generator <- function(marker_list) {
  ...
  timeseries_marker_list <- lapply(seq_along(marker_list), function(x) {
    ...
    timeseries_data %>% 
      ...
      geom_line(aes(group = Patient_SampleID, linetype = Outcome_Group, colour = Outcome_Group), lwd = 1.5) +
      geom_point(aes(colour = Outcome_Group, shape = Relapse, size = Relapse)) +
      ...
      ggtitle(paste("Temporal dynamics of", sub(".*[%] *(.*?) *[(].*", "\\1", title)),
              subtitle = paste0('\u0394 ', 'Group p = ', format(Group_pvals[tmp], digits = 1, nsmall = 3))) +
...
composition_arranged <- wrap_elements(full = cross_sectional_composition$`%CD4+ T cells (CD3+CD4+)` + coord_cartesian(ylim = c(0, 125))) + wrap_elements(full = longitudinal_composition$`%CD4+ T cells (CD3+CD4+)`) +
  wrap_elements(full = cross_sectional_composition$`%CD8+ T cells (CD3+CD8+)` + coord_cartesian(ylim = c(0, 145))) + wrap_elements(full = longitudinal_composition$`%CD8+ T cells (CD3+CD8+)`) +
  wrap_elements(full = cross_sectional_composition$`%NK cells (CD3-CD56+CD16+)` + coord_cartesian(ylim = c(0, 75))) + wrap_elements(full = longitudinal_composition$`%NK cells (CD3-CD56+CD16+)`) +
...
# ggsave(here("analyses", "interim_output", 'Figure2_Flowcyto_composition_CvR.pdf'), composition_arranged, dev = cairo_pdf, height = 17.5, width = 32.5)
...
cross_sectional_CD4_functional <- Cross_sectional_boxplot_generator(CD4_functional_list, step_size_list = c(0.175, 0.225))
...
arranged_CD4_functional <- wrap_elements(full = cross_sectional_CD4_functional$`%IFNg+ CD4+ T cells (IFNg+/CD3+CD4+)` + coord_cartesian(ylim = c(0,142.5))) + wrap_elements(full = longitudinal_CD4_functional$`%IFNg+ CD4+ T cells (IFNg+/CD3+CD4+)`) +
  wrap_elements(full = cross_sectional_CD4_functional$`%Th1 cells (IFNg+/CXCR3+/CD3+CD4+)` + coord_cartesian(ylim = c(0,142.5))) + wrap_elements(full = longitudinal_CD4_functional$`%Th1 cells (IFNg+/CXCR3+/CD3+CD4+)`) +
  plot_layout(design = layout) + plot_annotation(tag_levels = c('a')) & theme(plot.tag = element_text(size = 36, face = 'bold', vjust = 2))

ggsave(here("analyses", "final_output", "supplemental", "SuppFigure2_Flowcyto_Th1.pdf"), arranged_CD4_functional, dev = cairo_pdf, height = 12.5, width = 35)
ggsave(here("analyses", "final_output", "supplemental", "SuppFigure2_Flowcyto_Th1.png"), arranged_CD4_functional, type = 'cairo-png', height = 12.5, width = 35)
...
cross_sectional_CD4_exhaustion <- Cross_sectional_boxplot_generator(CD4_exhaustion_list, step_size_list = c(0.15, 0.075, 0.075, 0.125))
...
arranged_CD4_exhaustion <- wrap_elements(full = cross_sectional_CD4_exhaustion$`%PD1+ CD8- T cells (PD1+/CD3+CD8-)` + coord_cartesian(ylim = c(0,147.5))) + wrap_elements(full = longitudinal_CD4_exhaustion$`%PD1+ CD8- T cells (PD1+/CD3+CD8-)`) +
  wrap_elements(full = cross_sectional_CD4_exhaustion$`%LAG3+ CD8- T cells (LAG3+/CD3+CD8-)` + coord_cartesian(ylim = c(0,70))) + wrap_elements(full = longitudinal_CD4_exhaustion$`%LAG3+ CD8- T cells (LAG3+/CD3+CD8-)`) +
  wrap_elements(full = cross_sectional_CD4_exhaustion$`%TIM3+ CD8- T cells (TIM3+/CD3+CD8-)` + coord_cartesian(ylim = c(0,28.5))) + wrap_elements(full = longitudinal_CD4_exhaustion$`%TIM3+ CD8- T cells (TIM3+/CD3+CD8-)`) +
  wrap_elements(full = cross_sectional_CD4_exhaustion$`%TIGIT+ CD8- T cells (TIGIT+/CD3+CD8-)` + coord_cartesian(ylim = c(0,145))) + wrap_elements(full = longitudinal_CD4_exhaustion$`%TIGIT+ CD8- T cells (TIGIT+/CD3+CD8-)`) +
  plot_layout(design = layout) + plot_annotation(tag_levels = c('a')) & theme(plot.tag = element_text(size = 36, face = 'bold'))

# ggsave(here("analyses", "final_output", "Figure5_Flowcyto_CD4Exhaustion.pdf"), arranged_CD4_exhaustion, dev = cairo_pdf, height = 22.5, width = 35)
# ggsave(here("analyses", "final_output", "Figure5_Flowcyto_CD4Exhaustion.png"), arranged_CD4_exhaustion, type = 'cairo-png', height = 22.5, width = 35)
...
arranged_CD8_functional <- wrap_elements(full = cross_sectional_CD8_functional$`%IFNg+ CD8+ T cells (IFNg+/CD3+CD8+)` + coord_cartesian(ylim = c(0,122.5))) + wrap_elements(full = longitudinal_CD8_functional$`%IFNg+ CD8+ T cells (IFNg+/CD3+CD8+)`) +
  wrap_elements(full = cross_sectional_CD8_functional$`%IL17+ CD8+ T cells (IL17+/CD3+CD4-)`  + coord_cartesian(ylim = c(0,57.5))) + wrap_elements(full = longitudinal_CD8_functional$`%IL17+ CD8+ T cells (IL17+/CD3+CD4-)`) +
  wrap_elements(full = cross_sectional_CD8_functional$`%CD107+ CD8+ T cells (CD107+/CD3+CD8+)` + coord_cartesian(ylim = c(0,100))) + wrap_elements(full = longitudinal_CD8_functional$`%CD107+ CD8+ T cells (CD107+/CD3+CD8+)`) +
  plot_layout(design = layout) + plot_annotation(tag_levels = c('a')) & theme(plot.tag = element_text(size = 36, face = 'bold'))

# ggsave(here("analyses", "final_output", "Figure6_Flowcyto_CD8Functional.pdf"), arranged_CD8_functional, dev = cairo_pdf, height = 17.5, width = 35)
# ggsave(here("analyses", "final_output", "Figure6_Flowcyto_CD8Functional.png"), arranged_CD8_functional, type = 'cairo-png', height = 17.5, width = 35)
...
cross_sectional_CD8_exhaustion <- Cross_sectional_boxplot_generator(CD8_exhaustion_list, step_size = c(0.175, 0.125, 0.15, 0.2))
...
arranged_CD8_exhaustion <- wrap_elements(full = cross_sectional_CD8_exhaustion$`%PD1+ CD8+ T cells (PD1+/CD3+CD8+)` + coord_cartesian(ylim = c(0,165))) + wrap_elements(full = longitudinal_CD8_exhaustion$`%PD1+ CD8+ T cells (PD1+/CD3+CD8+)`) +
  wrap_elements(full = cross_sectional_CD8_exhaustion$`%LAG3+ CD8+ T cells (LAG3+/CD3+CD8+)` + coord_cartesian(ylim = c(0,125))) + wrap_elements(full = longitudinal_CD8_exhaustion$`%LAG3+ CD8+ T cells (LAG3+/CD3+CD8+)`) +
  wrap_elements(full = cross_sectional_CD8_exhaustion$`%TIM3+ CD8+ T cells (TIM3+/CD3+CD8+)` + coord_cartesian(ylim = c(0,23.5))) + wrap_elements(full = longitudinal_CD8_exhaustion$`%TIM3+ CD8+ T cells (TIM3+/CD3+CD8+)`) +
  wrap_elements(full = cross_sectional_CD8_exhaustion$`%TIGIT+ CD8+ T cells (TIGIT+/CD3+CD8+)` + coord_cartesian(ylim = c(0,160))) + wrap_elements(full = longitudinal_CD8_exhaustion$`%TIGIT+ CD8+ T cells (TIGIT+/CD3+CD8+)`) +
  plot_layout(design = layout) + plot_annotation(tag_levels = c('a')) & theme(plot.tag = element_text(size = 36, face = 'bold'))

# ggsave(here("analyses", "final_output", "Figure7_Flowcyto_CD8Exhaustion.pdf"), arranged_CD8_exhaustion, dev = cairo_pdf, height = 22.5, width = 35)
# ggsave(here("analyses", "final_output", "Figure7_Flowcyto_CD8Exhaustion.png"), arranged_CD8_exhaustion, type = 'cairo-png', height = 22.5, width = 35)
...
arranged_Supp_Senescence <- wrap_elements(full = cross_sectional_Supp_Senescence$`%KLRG1+ CD8- T cells (KLRG1+/CD3+CD8-)` + coord_cartesian(ylim = c(0,127.5))) + wrap_elements(full = longitudinal_Supp_Senescence$`%KLRG1+ CD8- T cells (KLRG1+/CD3+CD8-)`) +
  wrap_elements(full = cross_sectional_Supp_Senescence$`%CD57+ CD8- T cells (CD57+/CD3+CD8-)` + coord_cartesian(ylim = c(0,105))) + wrap_elements(full = longitudinal_Supp_Senescence$`%CD57+ CD8- T cells (CD57+/CD3+CD8-)`) +
  wrap_elements(full = cross_sectional_Supp_Senescence$`%KLRG1+ CD8+ T cells (KLRG1+/CD3+CD8+)` + coord_cartesian(ylim = c(40,142.5))) + wrap_elements(full = longitudinal_Supp_Senescence$`%KLRG1+ CD8+ T cells (KLRG1+/CD3+CD8+)`) +
  wrap_elements(full = cross_sectional_Supp_Senescence$`%CD57+ CD8+ T cells (CD57+/CD3+CD8+)` + coord_cartesian(ylim = c(10,140))) + wrap_elements(full = longitudinal_Supp_Senescence$`%CD57+ CD8+ T cells (CD57+/CD3+CD8+)`) +
  plot_layout(design = layout) + plot_annotation(tag_levels = c('a')) & theme(plot.tag = element_text(size = 36, face = 'bold'))

# ggsave(here("analyses", "final_output", "supplemental", "SuppFigure2_Flowcyto_Senescence.pdf"), arranged_Supp_Senescence, dev = cairo_pdf, height = 22.5, width = 35)
# ggsave(here("analyses", "final_output", "supplemental", "SuppFigure2_Flowcyto_Senescence.png"), arranged_Supp_Senescence, type = 'cairo-png', height = 22.5, width = 35)
...
arranged_Supp_NK <- wrap_elements(full = cross_sectional_Supp_NK$`%IFNg+ NK cells (IFNg+/CD3-CD56+CD16+)` + coord_cartesian(ylim = c(0,102.5))) + wrap_elements(full = longitudinal_Supp_NK$`%IFNg+ NK cells (IFNg+/CD3-CD56+CD16+)`) + 
  wrap_elements(full = cross_sectional_Supp_NK$`%CD107+ NK cells (CD107+/CD3-CD56+CD16+)` + coord_cartesian(ylim = c(0,85))) + wrap_elements(full = longitudinal_Supp_NK$`%CD107+ NK cells (CD107+/CD3-CD56+CD16+)`) +
  wrap_elements(full = cross_sectional_Supp_NK$`%TIGIT+ NK cells (TIGIT+/CD3-CD56+CD16+)` + coord_cartesian(ylim = c(0,137.5))) + wrap_elements(full = longitudinal_Supp_NK$`%TIGIT+ NK cells (TIGIT+/CD3-CD56+CD16+)`) +
  plot_layout(design = layout) + plot_annotation(tag_levels = c('a')) & theme(plot.tag = element_text(size = 36, face = 'bold'))

# ggsave(here("analyses", "final_output", "supplemental", "SuppFigure3_Flowcyto_NK.pdf"), arranged_Supp_NK, dev = cairo_pdf, height = 20, width = 35)
# ggsave(here("analyses", "final_output", "supplemental", "SuppFigure3_Flowcyto_NK.png"), arranged_Supp_NK, type = 'cairo-png', height = 20, width = 35)